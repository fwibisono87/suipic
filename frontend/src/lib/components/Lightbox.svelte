				...
				<div class="flex items-start justify-between gap-2 sm:gap-4 text-white">
					<div class="flex-1 min-w-0">
						...
						{#if currentPhoto.description}
							<p class="text-xs sm:text-sm mt-1 sm:mt-2 opacity-90 line-clamp-2">{currentPhoto.description}</p>
						{/if}
						<div class="flex flex-wrap items-center gap-2 sm:gap-3 mt-2 sm:mt-3 text-xs sm:text-sm opacity-70">
							{#if currentPhoto.dateTaken}
								<div class="flex items-center gap-1">
									<Icon icon="mdi:calendar" class="text-sm" />
									<span class="hidden sm:inline">{new Date(currentPhoto.dateTaken).toLocaleDateString()}</span>
									<span class="sm:hidden">{new Date(currentPhoto.dateTaken).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
								</div>
							{/if}
							{#if currentPhoto.location}
								<div class="flex items-center gap-1">
									<Icon icon="mdi:map-marker" class="text-sm" />
									<span class="truncate max-w-[120px] sm:max-w-none">{currentPhoto.location}</span>
								</div>
							{/if}
						</div>
					</div>
