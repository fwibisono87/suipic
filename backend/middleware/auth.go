...
func AlbumAccess(authService *services.AuthService, requireEdit bool) fiber.Handler {
	return func(c *fiber.Ctx) error {
		if err := AuthRequired(authService)(c); err != nil {
			return err
		}
		...
		albumID, err := c.ParamsInt("album_id")
		if err != nil {
		...
		canAccess, err := authService.CanAccessAlbum(userID, int64(albumID), requireEdit)
		if err != nil {
		...
		if !canAccess {
			if requireEdit {
				return fiber.NewError(fiber.StatusForbidden, "edit access denied for this album")
			}
			return fiber.NewError(fiber.StatusForbidden, "access denied for this album")
		}
